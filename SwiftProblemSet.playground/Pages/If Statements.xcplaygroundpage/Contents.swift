...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs < 10 && music && levels > 5 {
        ...
        if bugs >= 10 {
            print("Need less than 10 bugs to release")
        }
        if !music {
            print("Need to have music to release")
        }
        if levels <= 5 {
            print("Need to have more than 5 levels to release")
        }
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if bike && run {
        ...
        print("\(name) need to finish biking.")
    } else {
        print("\(name) need to finish running.")
    }
}
